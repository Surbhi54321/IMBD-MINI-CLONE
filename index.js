	...
	for(let i = 0; i<currentMovieStack.length; i++){
		...
		<div class="movie-title-container">
			...
			<div class="rating-container">
				<img src="./project-Icone/4001136.png" alt="rating">
		...
		<button onclick="${actionForButton}(this)" class="add-to-favourite-button text-icon-button" data-id="${currentMovieStack[i].id}" >
			<img src="./project-Icone/download.jpg">
...
function favourite(element){
	...
	for(let i = 0; i< currentMovieStack.length; i++){
		if(currentMovieStack[i].id == id){
			let favouriteMovie = JSON.parse(localStorage.getItem("favouriteMovie"));
			
			if(favouriteMovie == null){
				favouriteMovie = [];
			}

			favouriteMovie.unshift(currentMovieStack[i]);
...
goToFavouriteButton.addEventListener('click', ()=>{
	let favouriteMovie = JSON.parse(localStorage.getItem("favouriteMovie"));
	if(favouriteMovie == null || favouriteMovie.length < 1){
		showAlert("you have not added any movie to favourite");
		return;
	}

	currentMovieStack = favouriteMovie;
	renderList("remove");
})
...
function remove(element){
	...
	for(let i = 0; i<favouriteMovie.length; i++){
		if(favouriteMovie[i].id == id){
			continue;
		}
		newFavouriteMovies.push(favouriteMovie[i]);
	}
	
	localStorage.setItem("favouriteMovie", JSON.stringify(newFavouriteMovies));
